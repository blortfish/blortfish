...
export function PostList() {
  ...
  return (
    <div className="post-list">
      <h2 className="text-xl font-semibold mb-4 dark:text-gray-200">Posts</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data.posts.map((post: Post, index: number) => (
          <div key={index} className="border dark:border-dark-700 rounded-lg p-4 shadow-sm dark:bg-dark-800 transition-transform hover:scale-102 hover:shadow-md">
            <a href={getVideoUrl(post)} target="_blank" rel="noopener noreferrer" className="block overflow-hidden rounded">
              <img 
                src={getThumbnailUrl(post)} 
                alt={`${post.service} content by ${post.postedBy}`}
            ...
            <div className="mt-3">
              ...
              <span className="ml-2 text-sm dark:text-gray-400">
                by {post.postedBy} on {new Date(post.datePosted).toLocaleDateString()}
              </span>
            </div>
